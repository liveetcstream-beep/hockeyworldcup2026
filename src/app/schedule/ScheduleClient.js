...
export default function ScheduleClient() {
  ...
  const getConvertedTime = (timeCEST, tz) => {
    if (!timeCEST) return "";
    if (tz === "CEST" || tz === "CET") return `${timeCEST} CEST`;
  ...
  const filteredMatches = useMemo(() => {
    const list = ALL_MATCHES.filter((match) => {
      ...
      const matchesSearch = 
        ...
        (match.matchCode && match.matchCode.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return list.sort((a, b) => {
      ...
      const timeA = a.timeCEST || a.timeCET || "00:00";
      const timeB = b.timeCEST || b.timeCET || "00:00";
  ...
  return (
    <>
      ...
      <section id="fixtures-tabs" style={{ marginBottom: "2rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
          <h2 className="text-xl font-bold text-white">Official Tournament Fixtures (100 Matches)</h2>
            ...
            {paginatedMatches.map((match) => (
              <div key={match.id} className="match-card">
                <div className="match-meta">
                  <span>🗓️ {match.date} · <strong>{match.matchCode}</strong></span>
      ...
      <div className={`print-only-schedule print-mode-${printGender.toLowerCase()}`}>
        ...
        <div className="print-section-title">{printGender}'s Official Tournament Fixtures (50 Matches)</div>
          ...
          {ALL_MATCHES.filter(m => m.gender === printGender).map(match => (
            <div key={match.id} className={`print-match-card print-pool-${match.pool.replace(/\s+/g, '-').toLowerCase()}`}>
              <div className="print-match-meta">
                <span>🗓️ {match.date} · <strong>{match.matchCode}</strong></span>
                <span className="print-match-time">⏱️ {getConvertedTime(match.timeCEST || match.timeCET, selectedTimezone)}</span>
              </div>
              ...
              <div className="print-match-footer">
                ...
                <span>{match.matchCode} · {match.pool}</span>
              </div>
            </div>
          ))}
        </div>
